...
export default function App() {
  ...
  // Reference to keep track of books securely inside listeners without recreating them
  const booksRef = React.useRef<Record<string, Book>>(books);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchMissingBooks = async (bookIds: string[]) => {
      const missingIds = Array.from(new Set(bookIds)).filter(id => !booksRef.current[id]);
      if (missingIds.length === 0) return;

      const fetched = await Promise.all(missingIds.map(id => getBookById(id)));
      const newBooks = fetched.reduce((acc, book) => {
    ...
    const unsubscribeProgress = onSnapshot(q, (snapshot) => {
      ...
      fetchMissingBooks(progress.map(p => p.bookId));
    });
    ...
    const unsubscribeReviews = onSnapshot(qReviews, (snapshot) => {
      ...
      setReviews(revs);
      fetchMissingBooks(revs.map(r => r.bookId));
    });
            ...
            >
              <Tabs defaultValue="all" className="w-full">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                  <TabsList className="bg-white/50 p-1 rounded-2xl w-fit flex flex-wrap max-w-full overflow-x-auto">
                    <TabsTrigger value="all" className="rounded-xl px-4 md:px-8">All Books</TabsTrigger>
                    <TabsTrigger value="reading" className="rounded-xl px-4 md:px-8">Reading</TabsTrigger>
                    <TabsTrigger value="reading_list" className="rounded-xl px-4 md:px-8">Reading List</TabsTrigger>
                    <TabsTrigger value="wishlist" className="rounded-xl px-4 md:px-8">Wishlist</TabsTrigger>
                    <TabsTrigger value="completed" className="rounded-xl px-4 md:px-8">Completed</TabsTrigger>
                  </TabsList>
              ...
              <TabsContent value="reading_list">
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
                  {sortBooks(readingList).map(p => books[p.bookId] && (
                    <BookCard key={p.id} book={books[p.bookId]} onAction={handleBookAction} onClick={handleBookClick} />
                  ))}
